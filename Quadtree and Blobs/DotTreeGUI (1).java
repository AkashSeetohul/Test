...
public class DotTreeGUI extends DrawingGUI {
	...
	private int testFind(int x, int y, int r, int expectedCircleRectangle, int expectedInCircle, int expectedHits) {
		...
		if (num != expectedHits) {
			...
			System.out.println(tree.findInCircle(x, y, r));
		}
		return errs;
	}
	...
	private void test2() {
		...
		int bad = 0;
		bad += testFind(500,400,50,4,4,1); // rectangle for all; circle for all; find C
		bad += testFind(450, 350, 10, 4,3,1); //rectangle for all; circle for A, B, C; find B
	...
	public void drawTree(Graphics g, PointQuadtree<Dot> tree, int level) {
		...
		level = tree.getHeight();			// update level to be the height of the tree
